...
module iteration_simple # (
    ...
    poly_axis qr_decomp_in;
    poly_axis qr_decomp_out;

    poly_axis s[QR_DECOMP_CYCS:0];
    assign s[0] = qr_decomp_in;
    genvar i;
    generate
        for (i = 1; i <= QR_DECOMP_CYCS; i = i + 1) begin
    ...
    always_comb begin
        ...
        if (output_from_batch) begin
            ...
            for (int i = 0; i < MAX_N; i = i + 1) begin
                out_reg.meta.x[i] = qr_decomp_out.meta.a[i];
            end
        end else begin
            out_reg.valid = 0;
        end
